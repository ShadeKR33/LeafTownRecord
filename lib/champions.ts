export const CHAMPION_LIST = [
  ...
  "다리우스", "다이애나", "드레이븐", "라이즈", "라칸", "람머스", "럭스", "럼블", "레나타 글라스크", "레넥톤", "레오나", "렉사이", "렐", "렝가", "로크", "루시안", "룰루", "르블랑",
...
// 챔피언 이름 띄어쓰기 변형(예: "신짜오" ↔ "신 짜오", "자르반4세" ↔ "자르반 4세")을
// 같은 챔피언으로 통합하기 위한 정규화. 통계 집계 키, 입력값 보정 등에 공통으로 사용한다.
const noSpace = (s: string) => s.replace(/\s+/g, "");

let _noSpaceMap: Map<string, string> | null = null;
function getNoSpaceMap(): Map<string, string> {
  if (!_noSpaceMap) {
    _noSpaceMap = new Map();
    CHAMPION_LIST.forEach((name) => _noSpaceMap!.set(noSpace(name), name));
  }
  return _noSpaceMap;
}
...
export function normalizeChampionName(raw: string): string {
  ...
  const canonical = getNoSpaceMap().get(noSpace(trimmed));
